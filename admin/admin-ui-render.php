...
/**
 * Callback function for creating checkbox input fields
 * 
 * @since 0.1.0
 * @param Array $args contains a list of settings in the following order [$settings_id, $description]
 */
function loginid_dw_checkbox_input_field_callback($args)
{
  // destructure args
  [$settings_id, $description] = $args;
  ...
  <fieldset>
    <!-- Text Input -->
    <input type="checkbox" name="loginid_dw_settings[<?php echo esc_attr($settings_id) ?>]" id="<?php echo $settings_id ?>" value="1" <?php echo checked(1, $settings[$settings_id], true) ?> />
    <p class="description"><?php esc_html_e($description, 'loginid-directweb'); ?></p>
  </fieldset>
<?php
}
